...
gml_test_assert( GML_Installer::DB_VERSION === '3.0.1', 'pins the reviewed durable resource-readiness database version' );
...
gml_test_assert( substr_count( $source, 'ALTER TABLE' ) === 1 && strpos( $source, 'ALTER TABLE $table ADD KEY status_id (status, id)' ) !== false, 'upgrade only adds the bounded readiness worker index' );
