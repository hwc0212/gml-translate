...
The fixture uses real renderer, URL helpers, CSS and JS. It has no database, credentials or provider calls. Its header deliberately uses `overflow:hidden` and a fixed height.

At widths 1440, 768 and 390, open `/?case=all` and `/staging/?case=all`. Verify collapsed/expanded ARIA, five alternatives (four local plus one external), body-level panel placement, hit-testing every link, no header height change, no horizontal overflow, and outside-click closing. The external example is a fixture destination, not a live site.
