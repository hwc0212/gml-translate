  ...
  try {
    for (const file of fs.readdirSync(root).filter(f => f.endsWith('.html'))) {
      for (const width of [1440,768,390]) {
        ...
        assert((await button.innerText()).startsWith(file.endsWith('-de.html')?'DE':'EN'),'current language label');
        const currentStyle=await button.evaluate(el=>({color:getComputedStyle(el).color,font:parseFloat(getComputedStyle(el).fontSize)}));
        assert(currentStyle.color!=='rgb(255, 255, 255)' && currentStyle.font>=12,'current language visible against white header');
        await button.focus();
