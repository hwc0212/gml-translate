...
- Core 0.4.10 将 HTTP 429、5xx、网络和超时错误改为自动冷却，当前任务退回 pending 且不消耗重试次数；HTTP 400、认证、权限和模型错误继续安全暂停。
- 后台区分新增失败与已确认的历史失败，按类型汇总并可展开查看最近 20 条脱敏明细；不把 70,256 条旧记录误称为当前 API 请求。
- 成功连接测试只确认历史记录，不自动恢复队列；按语言重试仍限制每次最多 25 条，并先跳过翻译记忆中已经解决的记录。
- 语言完成度达到 95% 后，少量历史失败不再让整种语言全部失去 hreflang/index readiness；单页仍须满足 SEO 关键文本及 95% 页面翻译完整度。
- 新发现的缺失文本立即失效 readiness 与页面缓存，避免继续输出旧的索引信号；不修改数据库表、旧译文、URL、暂停状态或 API Key。
- 增加 WordPress 7.1 / MariaDB 10.11 的冷却恢复、熔断、历史错误分类、HTML 转义和零外部 API 请求回归。

## [2.11.1-rc.7] - 2026-08-31

### Gemini 测试与缓存确认候选版

- 页面缓存刷新增加输入 REFRESH 的确认框，服务端校验覆盖当前及旧版缓存操作；不删译文或队列。
- Core 0.4.9 共用 Gemini 最终答案解析，支持多片段、跳过思考内容，拒绝空答案与截断结果。
- 修复过低的连接测试输出上限，测试预算统一为 1024 token，取消测试请求的自动重试。
- 区分结束原因、token 上限、内容拦截与空响应，诊断不含密钥或原始响应文本。
- 补充确认失败零改动、缓存刷新数据保留、模拟 Gemini 响应、SEO 连接控制器和中英文弹窗回归。
- 保持模型、暂停、队列、路由和已有翻译数据；没有自动恢复生产翻译。

## [2.11.1-rc.6] - 2026-08-31

### 翻译控制分离候选版

- 修复小样本独占队列，导致全局及其他语言按钮不可用的问题；共用 Core 0.4.8。
- 顶部固定控制全部正常待翻译任务，各语言行独立控制；失败样本使用自己的暂停/继续入口，保留 25 条上限。
- 全局暂停后只启动一个语言，不会误启动其他语言；样本完成不会暂停用户已启动的正常任务。
- 正常任务排除被隔离的样本和历史失败记录；禁用语言不参与处理，原译文和队列记录不迁移、不删除。
- 修复一次性排程执行后不再继续的问题，保留每批一个 API 请求组、语言轮转与熔断。
- 补充真实数据库控制范围、旧样本升级、调度失败、独立扫描和中英文后台按钮回归。

## [2.11.1-rc.5] - 2026-08-31

### 小样本暂停恢复候选版

- 修复受限翻译样本暂停后，全局和语言开始按钮均禁用、无法继续的问题。
- 新增同一组最多 25 条样本的继续入口，显示语言和剩余条目，完成后仍自动暂停。
- Core 0.4.7 统一校验权限、nonce、Key、启用状态、熔断和工作锁；排程成功后才解除样本暂停。
- 不清除样本边界，不重置进度/次数，不开启全量重试，不修改其他语言的暂停状态。
- 补充中英文后台、部分完成后反复暂停/继续、受限调度失败和数据保留回归。

## [2.11.1-rc.4] - 2026-08-31

### 路由与队列修复候选版

- 修复晚于 `init` 的激活未注册语言路由，及既有语言规则丢失后无法恢复的问题。
- 有权限的普通后台请求仅修复 GML 语言规则，保留其他规则；不增加前台、AJAX、Cron 的路由写入。
- Core 0.4.6 按语言轮转队列，保留每批数量、锁、暂停、熔断和小样本保护，不重启或清空队列。
- 增加真实 WordPress 根目录/子目录、无 Key、规则丢失、关闭/重启多语言、真实 404、队列轮转与暂停回归。
- 不更换数据库格式、不删除译文；路由修复不代表翻译覆盖完成。

## [2.11.1-rc.3] - 2026-08-31

### 凭据修复候选版

- Core 0.4.5：解密失败不再把密文当作 API Key 发给服务商；写入须成功且回读一致，失败不得误报成功。
- Save Changes 只做本地加密保存与回读；Test Saved AI Connection 单独读取已保存值并做一次连接测试，不隐藏发起两次 AI 请求。
- 分离原先嵌套在主设置中的语言排序表单，避免保存设置时混入排序 action；保存与测试按钮各自绑定独立表单。
- 去掉密码框的 32 星号占位和聚焦自动明文显示；状态明确区分本地可读、不可解密与连接测试结果，不再用绿色“已配置”暗示 API 可用。
- 修复只存在 DeepSeek Key 时误认为 Gemini 已配置。换 Key、切换引擎或重新启用 AI 均保留暂停，测试成功也不会自动恢复翻译。
- 不更改旧密文格式、options、翻译表、人工译文或 URL。未经识别的旧明文记录需重新输入；新的不透明格式 Key 可正常加密保存。
- 增加真实 WordPress 保存→测试、写入失败、盐值不匹配、权限、nonce 和队列暂停回归；错误信息脱敏也覆盖不透明 Key。
- 只证明代码及隔离环境回归通过；尚未确认 cnxhe.com 的实际 400 根因，不宣称线上 API 或翻译覆盖已经恢复。

## [2.11.1-rc.2] - 2026-08-31

### 修复候选版

- 共享 Core 升至 0.4.4，保留 RC1 的数据库升级保护，不改名、清理或重建已有翻译表。
- 拆分内容扫描与 AI 队列控制。扫描仅入队，尊重现有暂停、熔断和小样本保护，不直接触发付费翻译。
- 修复误导性 Running 状态，区分排程与真实批次活动；扫描状态轮询不再整页刷新。
- 修复旧 Clear All Cache / Clear Language Cache 删除译文和队列的危险行为，改为无损页面缓存刷新；删除译文仅保留编辑器逐条确认入口。
- 修复技术参数中的星号、指数与 SKU 符号被 Markdown 清理误删，覆盖正文、标题、Meta、ALT 及 Provider 输出。
- 补充真实数据库权限/nonce、扫描与暂停独立、缓存数据保留、尺寸渲染回归和 WordPress 7.1 后台交互检查。
- 本地候选版，不宣称 cnxhe.com 已恢复、多语言覆盖已补齐或真实 Provider/生产主题/CDN 已验收。
- 浏览器验收补获同一请求内切换插件的加载冲突：独立版拒绝在已加载的 SEO 翻译宿主上二次激活，SEO 也不在已加载的独立宿主上再加载内置引擎；不自动删除或停用另一插件。
